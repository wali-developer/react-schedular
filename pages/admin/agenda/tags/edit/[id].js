...
const Index = ({ tagId }) => {
    ...
    const getTagDetails = async () => {
        setLoading(true)
        try {
            const { data } = await API.get(`/api/agenda/tag/${tagId}/`, header);
            if (data) {
                setName(data?.name)
                setColor(data?.color)
                setLoading(false)
    ...
    const updateTag = async () => {
        ...
        try {
            const { data } = await API.patch(`/api/agenda/tag/${tagId}/`, payLoad, header);
            if (data) {
                toast(`Tag ${data?.name} has successfully Updated :)`);
            }
            setLoading(false);
            setName('');
            setColor('');
            ...
            <Layout>
                <section className='flex-1 h-auto min-h-[450px] bg-white rounded-[32px] px-8 py-8'>
                    <div className='flex justify-between px-0 md:px-10'>
                        <h1 className='font-medium text-xl md:text-[24px]'>Edit Tag</h1>
                    </div>
                    <div className='w-full xl:w-[822px] mt-10 px-6 md:px-10'>
                        <form onSubmit={(e) => {
                            ...
                            <button
                                type='submit'
                                className='w-[180px] h-[48px] text-base text-white bg-primary rounded-xl font-light mt-4'
                            >Update Tag</button>
                        </form>
                    </div>
                </section>
